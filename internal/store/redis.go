...
func (r *RedisStore) CreateOrGetByKey(ctx context.Context, key string, t *models.Task) (*models.Task, bool, error) {
    ...
    if key != "" {
        if id, err := r.rdb.Get(ctx, r.prefix+":idem:"+key).Result(); err == nil {
            ...
            if err := json.Unmarshal([]byte(data), &existing); err != nil { return nil, false, err }
    ...
    if err := r.rdb.Set(ctx, r.key(t.ID), b, 0).Err(); err != nil { return nil, false, err }
    if key != "" {
        if err := r.rdb.Set(ctx, r.prefix+":idem:"+key, t.ID, 0).Err(); err != nil { return nil, false, err }
    }
    return t, false, nil
}
...
func (r *RedisStore) Get(ctx context.Context, id string) (*models.Task, error) {
    ...
    if err == redis.Nil { return nil, errRedisNotFound }
    if err != nil { return nil, err }
    var t models.Task
    if err := json.Unmarshal([]byte(s), &t); err != nil { return nil, err }
    return &t, nil
}
